...
class StaticGenerator {
  ...
  _generateHTMLFromElements() {
    ...
    function logChilds(element) {
      if (element.children.length === 0) {
        return `<${element.type}></${element.type}>`;
      }

      let HTMLString = `<${element.type}>`;
      /** Get children */
      for (let i = 0; i < element.children.length; i++) {
        HTMLString += logChilds(element.children[i]);
      }

      return HTMLString += `</${element.type}>`
    }

    console.log(logChilds(grandParent));
  }
