...
class StaticGenerator {
  ...
  _generateElementsFromTree() {
    ...
    console.log(this.treeObjectArray);

    let htmlString = '';
    let newLineTabs = '\t';
    function newLineTabsEdit(action) {
      if (action) {
        newLineTabs += '\t';
      } else {
        newLineTabs = newLineTabs.slice(0, newLineTabs.length-1);
      }

      return newLineTabs;
    }
    
    while (iterations < this.treeObjectArray.length) {
      function recursiveHTML(arr, curr, prePriority) {
        if (iterations === arr.length-1 || prePriority > curr.priority) 
          return '';      

        iterations++;
        return `<${curr.tag}>\n${recursiveHTML(arr, arr[iterations], curr.priority)}</${curr.tag}>`
      }
      htmlString += '\n';
      htmlString += recursiveHTML(this.treeObjectArray, this.treeObjectArray[iterations]);
      iterations++;
    }
